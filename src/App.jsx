...
import { useActiveNotes, useTimeline, useTimeOrigin, useAudioState } from './hooks/useEngine';
...
const handleNoteOn = (note, velocity = 90) => engine.noteOn(note, velocity, 'user');
const handleNoteOff = (note) => engine.noteOff(note, 'user');
...
function App() {
  ...
  const { events: timelineEvents, heldNotes, breaks, inBreak } = useTimeline();
  ...
  const detectedChord = useMemo(() => {
    ...
    if (activeNotes.size === 1) return midiToNoteName([...activeNotes][0]);
    const names = [...activeNotes].map(midiToNoteName);
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#15171c]">
      <div className="flex flex-1 flex-col min-w-0 overflow-hidden app-bg relative">
        ...
        <div className="flex items-center justify-between px-4 py-2 shrink-0 border-b border-[#2a2d35] bg-[#1a1c22]/80 z-20">
          ...
          <div className="flex items-center gap-4">
            ...
            {/* Legend */}
            <span className="flex items-center gap-1.5 text-[10px] font-bold tracking-wide uppercase text-gray-500">
              <span className="inline-block w-2.5 h-1.5 rounded-sm bg-blue-500" /> You
            </span>
            <span className="flex items-center gap-1.5 text-[10px] font-bold tracking-wide uppercase text-gray-500">
              <span className="inline-block w-2.5 h-1.5 rounded-sm bg-orange-500" /> AI
            </span>
          </div>
        </div>
      ...
      <div className="w-[340px] shrink-0 border-l-2 border-[#2a2d35] flex flex-col h-screen">
        <AiChat noteRecorder={recorderRef} onPlaySequence={engine.playSequence} speech={speech} />
      </div>
